...
public abstract class Char extends Actor {
	...
	public boolean interact(Char c){
		...
		if (c == Dungeon.hero){
			...
			Bicycle bicycle = ((Hero)c).belongings.getItem(Bicycle.class);
			if (bicycle != null) {
				Bicycle.BicycleBuff bicycleBuff = c.buff(Bicycle.BicycleBuff.class);
				if (bicycleBuff != null) {
					bicycle.onRide();
		...
		if (enemy.isInvulnerable(getClass())) {
			...
			if (this instanceof Hero){
				...
				if (h.belongings.attackingWeapon() instanceof Gun.Bullet) {
					dr = Math.round(dr * ((Gun.Bullet) h.belongings.attackingWeapon()).whatBullet().armorFactor());
				}
			...
			SwiftMovement.WeaknessTracker weakness = enemy.buff(SwiftMovement.WeaknessTracker.class);
			if (weakness != null && this instanceof Hero){
				if ( enemy.isAlive() && enemy.alignment != alignment && !Char.hasProp(enemy, Property.BOSS)
						&& !Char.hasProp(enemy, Property.MINIBOSS) &&
						(enemy.HP/(float)enemy.HT) <= 0.4f*((Hero)this).pointsInTalent(Talent.HOSHINO_EX2_2)/3f) {
					enemy.HP = 0;
					if (enemy.buff(Brute.BruteRage.class) != null){
						enemy.buff(Brute.BruteRage.class).detach();
					}
	...
	public static boolean hit( Char attacker, Char defender, float accMulti, boolean magic ) {
		...
		if (defender.buff(TrashBin.EvasionBuff.class) != null){
			defStat = INFINITE_EVASION;
		}
	...
	public void damage( int dmg, Object src ) {
		...
		int shielded = dmg;
		dmg = AvantGardeKunBuff.processDamage(this, dmg, src);
